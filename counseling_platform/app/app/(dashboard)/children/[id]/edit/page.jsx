...
function EditChildPage({ params }) {
    ...
    const [isLoadingChild, setIsLoadingChild] = (0, react_1.useState)(true);
    const [newInterest, setNewInterest] = (0, react_1.useState)('');
    const [newChallenge, setNewChallenge] = (0, react_1.useState)('');
    (0, react_1.useEffect)(() => {
        fetchChild();
    }, [params.id]);
    const fetchChild = () => __awaiter(this, void 0, void 0, function* () {
        try {
            setIsLoadingChild(true);
            const response = yield fetch(`/api/children/${params.id}`);
            if (response.ok) {
