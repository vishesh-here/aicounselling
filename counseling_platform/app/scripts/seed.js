...
// Tags data
const tagsData = [
    { name: "Academic Support", category: "EDUCATIONAL", color: "#3B82F6" },
    { name: "Career Confusion", category: "ISSUE", color: "#EF4444" },
    { name: "Family Problems", category: "ISSUE", color: "#F59E0B" },
    { name: "Low Self-Esteem", category: "ISSUE", color: "#8B5CF6" },
    { name: "Financial Stress", category: "ISSUE", color: "#EC4899" },
    { name: "Mathematics", category: "INTEREST", color: "#10B981" },
    { name: "Science", category: "INTEREST", color: "#06B6D4" },
    { name: "Arts", category: "INTEREST", color: "#F97316" },
    { name: "Sports", category: "INTEREST", color: "#84CC16" },
    { name: "Technology", category: "INTEREST", color: "#6366F1" },
    { name: "Leadership", category: "SKILL", color: "#DC2626" },
    { name: "Communication", category: "SKILL", color: "#059669" },
    { name: "Problem Solving", category: "SKILL", color: "#7C3AED" },
    { name: "Teenage", category: "DEMOGRAPHIC", color: "#BE123C" },
    { name: "Primary School", category: "DEMOGRAPHIC", color: "#1D4ED8" },
    { name: "High School", category: "DEMOGRAPHIC", color: "#B91C1C" },
    { name: "Rural Background", category: "DEMOGRAPHIC", color: "#059669" },
    { name: "Urban Slum", category: "DEMOGRAPHIC", color: "#DC2626" },
    { name: "Single Parent", category: "DEMOGRAPHIC", color: "#7C3AED" },
    { name: "Economic Hardship", category: "ISSUE", color: "#EF4444" }
];
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            yield prisma.documentChunk.deleteMany();
            yield prisma.sessionSummary.deleteMany();
            yield prisma.session.deleteMany();
            yield prisma.chatMessage.deleteMany();
            yield prisma.concern.deleteMany();
            yield prisma.assignment.deleteMany();
            yield prisma.knowledgeBase.deleteMany();
            yield prisma.culturalStory.deleteMany();
            yield prisma.tag.deleteMany();
            yield prisma.child.deleteMany();
            ...
            for (const user of userData) {
                ...
                const createdUser = yield prisma.user.create({
                    data: Object.assign(Object.assign({}, user), { password: hashedPassword, 
                        ...
                        approvedBy: user.approvalStatus === "APPROVED" && user.role === "VOLUNTEER" ? undefined : undefined, approvedAt: user.approvalStatus === "APPROVED" ? new Date() : undefined })
                });
            ...
            const admin = yield prisma.user.findUnique({ where: { email: "admin@counseling.org" } });
            const approvedVolunteers = yield prisma.user.findMany({
                where: {
                    role: "VOLUNTEER",
                    approvalStatus: "APPROVED"
                }
            ...
            for (const story of culturalStories) {
                yield prisma.culturalStory.create({
                    data: {
                        title: story.title,
                        source: story.source,
            ...
            for (const entry of knowledgeBaseEntries) {
                const kbEntry = yield prisma.knowledgeBase.create({
                    data: Object.assign(Object.assign({}, entry), { createdById: admin.id })
                });
            ...
            for (const child of childProfiles) {
                const createdChild = yield prisma.child.create({
                    data: child
                });
            ...
            for (let i = 0; i < 35; i++) {
                ...
                const titles = concernTitles[randomCategory];
            ...
            for (let i = 0; i < 25; i++) {
                ...
                if (assignment) {
                    ...
                    if (randomStatus === "COMPLETED") {
                        ...
                        yield prisma.sessionSummary.create({
                            data: {
                                ...
                                resolutionStatus: randomResolution,
